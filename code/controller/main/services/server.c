...
{
    ESP_LOGI(SERVER_TAG, "Initializing SPIFFS");
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_FAIL) {
            ...
            ESP_LOGE(SERVER_TAG, "Failed to find SPIFFS partition");
        } else {
            ESP_LOGE(SERVER_TAG, "Failed to initialize SPIFFS (%s)", esp_err_to_name(ret));
        }
        return ESP_FAIL;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(SERVER_TAG, "Failed to get SPIFFS partition information (%s)", esp_err_to_name(ret));
        return ESP_FAIL;
    }

    ESP_LOGI(SERVER_TAG, "Partition size: total: %d, used: %d", total, used);
    return ESP_OK;
}
...
void sdcard_mount(void)
{
    ...
    ESP_LOGI(SERVER_TAG, "Initializing SD card");

#ifndef USE_SPI_MODE
    ESP_LOGI(SERVER_TAG, "Using SDMMC peripheral");
...
#else
    ESP_LOGI(SERVER_TAG, "Using SPI peripheral");
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(SERVER_TAG, "Failed to initialize bus.");
        ESP_ERROR_CHECK(ret);
    }
    ...
    if(ret != ESP_OK){
        if (ret == ESP_FAIL) {
            ESP_LOGE(SERVER_TAG, "Failed to mount filesystem. "
                "If you want the card to be formatted, set the EXAMPLE_FORMAT_IF_MOUNT_FAILED menuconfig option.");
        } else {
            ESP_LOGE(SERVER_TAG, "Failed to initialize the card (%s). "
