...
void PCD_WriteRegisterMany(uint8_t Register, uint8_t count, uint8_t *values){
	...
	esp_err_t ret;
	uint8_t *total = malloc(sizeof(uint8_t) * (count+1));
...
void PCD_ReadRegisterMany(
	...
	t.tx_buffer = 0;
	ret = spi_device_transmit(spi,&t);
	values[index] = val[0];			// Read the final byte. Send 0 to stop reading.
}
...
bool PCD_PerformSelfTest() {
	...
	PCD_WriteRegisterMany(FIFODataReg, 25, ZEROES); // write 25 bytes of 00h to FIFO
	...
	byte result[64];
	PCD_ReadRegisterMany(FIFODataReg, 64, result, 0);
...
uint8_t PCD_CommunicateWithPICC(
	...
	for (i = 20000; i > 0; i--) {
		...
		if ((n & 0xFF) & waitIRq) {                  // One of the interrupts that signal success has been set.
			//			printf("\n\nPCD_CommunicateWithPICC: --INTERRUPT- command %d, waitIRq 0x%x, n 0x%x, txLastBits 0x%x, rxAlign 0x%x\n",command,waitIRq,n,txLastBits,rxAlign);
			break;
		}
		if ((n & 0xFF)  & 0x01) {                     // Timer interrupt - nothing received in 25ms
	...
	if (backData && backLen) {
		...
		//		printf("Number of bytes in the FIFO n %d \n\n",n);
		if (n > *backLen) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
uint8_t PICC_WakeupA(byte * bufferATQA, ///< The buffer to store the ATQA (Answer to request) in
...
uint8_t PICC_REQA_or_WUPA(uint8_t command,uint8_t *bufferATQA,uint8_t *bufferSize){
	...
	validBits = 7;                                      // For REQA and WUPA we need the short frame format - transmit only 7 bits of the last (and only) uint8_t. TxLastBits = 	 [2..0]
...
StatusCode PCD_Authenticate(byte command, ///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
	...
	retVal= PCD_CommunicateWithPICC(PCD_MFAuthent, waitIRq, &sendData[0], sizeof(sendData),null,null,null,0,false);
	return retVal;
...
StatusCode MIFARE_Read(byte blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
	...
	result = PCD_TransceiveData(buffer, 4, buffer, bufferSize, null, 0, true);
...
StatusCode MIFARE_Restore(byte blockAddr ///< The block (0-0xff) number.
	...
	return MIFARE_TwoStepHelper(PICC_CMD_MF_RESTORE, blockAddr, 0);
} // End MIFARE_Restore()
...
bool PICC_ReadCardSerial() {
	...
	// printf("PICC_ReadCardSerial: Status %s\n",GetStatusCodeName(result));
	return (result == STATUS_OK);
} // End PICC_ReadCardSerial()

bool new_card_found() {
	bool value = cardFound;
	cardFound = false;
	return value;
}

void create_uid_string(uint8_t *buffer, uint8_t bufferSize) {
	strcpy(uid_str,"");
	for (uint8_t i = 0; i < bufferSize; i++) {
		sprintf(uid_str,"%s%x",uid_str,buffer[i]);
	}
}
...
void rc522_init(void * vParam) {
    ...
    printf("%x\n",PCD_ReadRegister(TxControlReg));
    printf("%x\n",PCD_ReadRegister(TxASKReg));
...
void rc522_main() {
	xTaskCreate( &rc522_init, "rc522_init", 8192, NULL, 5, NULL);
}
